...
$(function () {
    ...
    $.getJSON(url, function (data) {
        ...
        var info = Mustache.render(template1, statistics);
        $('#senate-data1').html(info);

        //        createTable2(names);
        var template2 = $("#user-template2").html();
        var info = Mustache.render(template2, statistics);
        $('#senate-data2').html(info);
        //        createTable3(names1);
...
function assignDataToStats(members) {
    parties = ["Democrats", "Republicans", "Independent", "Total"];
    ...
    var avrgTotal = (avrgPrct * (0.49) + avrgPrct1 * (0.49) + avrgPrct2 * (0.01)).toFixed(2);
...
function getTenPerct(members) {
    ...
    for (var i = 0; i < missedVotesPrct.length; i++) {
        ...
        var firstEl = missedVotesPrct[i].missed_votes_pct;
        var secondEl = missedVotesPrct[round - 1].missed_votes_pct;

        if (names.length < round || firstEl === secondEl) {
            var objLeast = {};
            objLeast.full_name = missedVotesPrct[i].first_name + " " + missedVotesPrct[i].last_name;
            objLeast.missed_votes = missedVotesPrct[i].missed_votes;
            objLeast.missed_votes_pct = missedVotesPrct[i].missed_votes_pct;
    ...
    for (var i = 0; i < missedVotesPrct.length; i++) {
        ...
        if (names1.length < round || firstEl === secondEl) {
            ...
            objMost.missed_votes = missedVotesPrct[i].missed_votes;
            objMost.missed_votes_pct = missedVotesPrct[i].missed_votes_pct;
...
function createTable2(membersArray) {
    ...
    for (var i = 0; i < membersArray.length; i++) {
        if (i == 0) {
            ...
            var thEl1 = document.createElement("th");
            trEl.append(thEl1);
            thEl1.textContent = "No. Missed Votes  ";
            var thEl2 = document.createElement("th");
            trEl.append(thEl2);
            thEl2.textContent = "% Missed  ";
        }
        var trEl1 = document.createElement("tr");
        tbodyEl.append(trEl1);
        ...
        var tdEl1 = document.createElement("td");
        trEl1.append(tdEl1);
        var calc = parseFloat(membersArray[i].total_votes / 100 * membersArray[i].votes_with_party_pct).toFixed(2);
        tdEl1.textContent = membersArray[i].missed_votes;

        var tdEl2 = document.createElement("td");
        trEl1.append(tdEl2);
        tdEl2.textContent = membersArray[i].missed_votes_pct;
    }
};
